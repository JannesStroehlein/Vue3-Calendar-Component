...
export function filterEvents(events: CalendarEventInternal[], filters: FilterOptions): CalendarEventInternal[] {
  ...
  if (filters.statuses && filters.statuses.length > 0) {
    filteredEvents = filteredEvents.filter((event) => filters.statuses!.indexOf(event.status || 'open') !== -1)
  }

  // Date range filter
  if (filters.dateRange) {
    const { start, end } = filters.dateRange
    filteredEvents = filteredEvents.filter((event) => event.startDate.isBefore(end) && event.endDate.isAfter(start))
  }
